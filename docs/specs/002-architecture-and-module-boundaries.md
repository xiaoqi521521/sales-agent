...
发送给大模型的上下文只包含 `user` 和 `assistant`，`tool` 消息用于追踪、响应摘要和持久化审计。

## 5. 错误与增强边界

在 Phase 0 规划语境中，认证、缓存、流式输出都被定义为后续增强能力，不阻塞基础 Agent 跑通。

当前实现状态备注：认证与流式输出已经在后续阶段落地；统一 API envelope、统一错误码、Redis 缓存、请求日志、token 成本统计仍属于生产化增强内容。
